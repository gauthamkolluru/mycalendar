...
export async function handleRequest(request, env, blobStore) {
  ...
  try {
    if (path === "/api/session" && method === "GET") {
      return requireSession(request, cfg)
        ? json(200, { ok: true })
...
function readConfig(env) {
  const password = env.CALENDAR_PASSWORD || "";
...
function requireSession(request, cfg) {
  ...
  return verifySession(cfg.secret, token);
}
...
async function login(request, cfg, blobStore) {
  ...
  if (!passwordsMatch(cfg.password, password)) {
    await bumpLogin(blobStore, ipKey);
    log.warn("login_failed");
    return json(401, { error: "Wrong password." });
  }
  await blobStore.setJSON(ipKey, { count: 0, resetAt: 0 });
  const token = signSession(cfg.secret, Date.now() + SESSION_MS);
  log.info("login_ok");
  return json(200, { ok: true }, { "set-cookie": sessionCookieHeader(token, isSecure(request)) });
}
...
async function listTasks(request, url, cfg, blobStore) {
  if (!requireSession(request, cfg)) return json(401, { error: "Sign in to open the calendar." });
  const parsed = parseYearMonth(url.searchParams.get("month") || "");
  if (!parsed.ok) return json(400, { error: parsed.error });
  const yearMonth = `${parsed.year}-${String(parsed.month).padStart(2, "0")}`;
  const tasks = await readMonth(blobStore, yearMonth);
  const grouped = {};
...
async function createTask(request, cfg, blobStore) {
  ...
  if (forDay.length >= MAX_TASKS_PER_DAY) {
    return json(409, { error: "That day is full." });
  }
  const task = {
    id: randomUUID(),
  ...
  tasks.push(task);
  await writeMonth(blobStore, yearMonth, tasks);
  log.info("task_created", { date: task.date });
...
async function removeTask(request, url, cfg, blobStore) {
  ...
  await writeMonth(blobStore, yearMonth, next);
  log.info("task_deleted", { date: url.searchParams.get("date") });
